...
const WatchPage = () => {
  ...
  return (
    ...
    <div className="space-y-2 2xl:space-y-5 lg:col-span-8">
      <iframe className="w-full md:w-full md:rounded-xl h-100 lg:h-120 2xl:h-220" src={`https://www.youtube.com/embed/${searchParams.get("v")}`} 
      ...
      <div className="grid md:grid-cols-2 gap-3 px-3 lg:px-0"> 
        ...
        <div className=" md:justify-self-start">
           ...
           <div className="flex items-center justify-between lg:justify-start lg:gap-6 2xl:gap-12">
            <div className="flex items-center gap-2 2xl:gap-5 ">
            <img className="w-8 h-8 2xl:w-12 2xl:h-12 rounded-full" src={snippet.thumbnails.default.url}/>
            <h2 className="text-xs xl:text-sm 2xl:text-3xl font-bold">{snippet.channelTitle}</h2>
            </div>
            <button onClick={()=> {
              handleSubscribeButton();
              }} className="text-xs md:text-sm 2xl:text-2xl bg-black text-white rounded-2xl 2xl:rounded-4xl px-4 py-1.5 2xl:px-8 2xl:py-4 hover:cursor-pointer">{subscribe ? "Subscribed " : "Subscribe"}</button>
           </div>
      ...
      {/* videos section */}
       <div className="flex flex-col gap-4 lg:col-span-4 w-full px-2 lg:px-0">
        ...
        {searchedVideos.length > 0 ? (
          ...
          <p className="text-gray-500 text-sm  px-2 text-center">No videos found</p>
        )}
      </div>
